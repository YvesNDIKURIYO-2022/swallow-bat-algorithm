...
# --- Custom Table Formatting Function ---
def format_table(headers, data):
    """Format data as a table without external dependencies"""
...
# ---  SBA Optimizer (Swallow Bat Algorithm) ---
class SBA:
    ...
    def _local_search(self, perm):
        ...
        if random.random() < 0.5:
            ...
            if i != j:
                customer = new_sol.pop(i)
                new_sol.insert(j, customer)
        return new_sol

# --- Bat Algorithm (BA) - Component Algorithm ---
class BatAlgorithm:
    def __init__(self, coords, demands, capacity, threat_zones, max_vehicles=5,
                 population_size=30, max_iter=200):
        self.coords = coords
...
# --- Particle Swarm Optimization (PSO) - Foundational Benchmark ---
class PSO:
    def __init__(self, coords, demands, capacity, threat_zones, max_vehicles=5,
...
# --- Harris Hawks Optimization (HHO) - State-of-the-Art Benchmark ---
class HHO:
    def __init__(self, coords, demands, capacity, threat_zones, max_vehicles=5,
                 population_size=30, max_iter=200):
        self.coords = coords
        ...
        for iteration in range(self.max_iter):
            ...
            for i in range(self.pop_size):
                ...
                if abs(E) >= 1:
                    ...
                    if random.random() >= 0.5:
                        # Random walk
                        new_sol = [int(self.population[i][j] * (1 + random.random())) % len(self.customers) + 1
                                  for j in range(len(self.customers))]
                    else:
                        # Based on other rabbits
                        k = random.randint(0, self.pop_size-1)
                        new_sol = [int((self.population[i][j] + self.population[k][j]) / 2) 
                                  % len(self.customers) + 1 for j in range(len(self.customers))]
                else:
                    # Exploitation phase
                    if random.random() >= 0.5:
                        # Soft besiege
                        new_sol = [int(self.best_solution[j] - E * abs(self.best_solution[j] - self.population[i][j]))
                                  for j in range(len(self.customers))]
                    else:
                        # Hard besiege
                        new_sol = [int(self.best_solution[j] - E * abs(self.best_solution[j] - self.population[i][j]))
                ...
                new_sol = [x % len(self.customers) + 1 for x in new_sol]
        ...
        while unserved:
            ...
            if best_route_idx is not None:
                ...
                if len(routes) < self.max_vehicles:
                    ...
                    # Force insertion in least loaded route
                    least_loaded_idx = min(range(len(routes)), 
                                         key=lambda i: sum(self.demands[c] for c in routes[i][1:-1]))
                    routes[least_loaded_idx].insert(1, customer)  # Insert after depot
...
class HGA:
    def __init__(self, coords, demands, capacity, threat_zones, max_vehicles=5,
                 population_size=30, max_generations=200, crossover_rate=0.8, 
...
def statistical_comparison(algorithms, coords, demands, capacity, threat_zones, max_vehicles, num_runs=30):
    """Run comprehensive statistical comparison of multiple algorithms"""
    print("=" * 80)
    ...
    # Perform ANOVA test if we have multiple algorithms
    if len(all_stats) > 1:
...
def main():
    ...
    # Define algorithms to compare (Updated list)
